...
export class CornetInfoLoadingHandler implements OnInit {
  ...
  constructor(private router: Router, private route: ActivatedRoute, private offenderService: OffenderService) {}
  ...
  getClientDetails(vsd_offenderid) {
    this.offenderService.getOffenderById(vsd_offenderid).subscribe(
      (res) => {
        ...
        if (res && res.value && res.value.length == 1) {
          ...
          if (this.coastInfo.vsd_csnumber) {
            ...
            this.router.navigate([`client-search`], {
              queryParams: {
                dosearch: true,
                surname: this.coastInfo.vsd_lastname,
                givenName: this.coastInfo.vsd_firstname,
                gender: this.coastInfo.vsd_gender,
                birthyear: this.coastInfo.vsd_birthdate ? this.coastInfo.vsd_birthdate.getFullYear() : null
              }
            });
          }
        } else {
