...
export class OffenderService {
  ...
  getOffenderByCSNumber(csNumber: string): Observable<any> {
    return this.http
      .get<any>(`${this.apiUrl}/cs-number/${csNumber}`, { headers: this.headers })
      .pipe(retry(3), catchError(this.handleError));
  }

  getOffenderById(offenderId: string): Observable<any> {
    return this.http
      .get<any>(`${this.apiUrl}/${offenderId}`, { headers: this.headers })
      .pipe(retry(3), catchError(this.handleError));
  }

  createOffender(offender: ICoastOffender): Observable<any> {
    return this.http
      .post<any>(`${this.apiUrl}`, offender, { headers: this.headers })
      .pipe(retry(3), catchError(this.handleError));
  }
